...
Here we first turn the precincts into factor so that R order is going to match the order in xgboost too, and then level them to get the precinct. Then we define y and x variable, which x is our predictor variable and y is the outcome. In the xgboost, we need to turn the factor as an interger, and then shift them by 1, because xgboost start from 0. Also, x will be the 21,209 rows by 2 matrixes, include x and y from the precious for loop(longitude and latitude). 
