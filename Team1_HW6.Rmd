...
Identify outliers of each precinct, so get rid of the 5% extreme values of each precinct (except for 22 since it only has 252 points) and sample 3% of the values. Instead, I consider precinct 22 separately
...
for (p in man_precincts) {
  if (p == 22) {
    ...
    res = nyc_geo[which(nyc_geo$precinct == p),]
    lon.quantile = unname(quantile(res$x, probs = c(0.025, 0.975)))
    lat.quantile = unname(quantile(res$y, probs = c(0.025, 0.975)))
...
```{r}
model = randomForest(as.factor(precinct)~x+y,
                     data=nyc_geo_reduced,
                     na.action = na.omit,
                     #ntree = 500,
                     #mtry = 2,
                     cross = 5)
idx22 = which((pred_locs[,'x'] >= x.min & pred_locs[,'x'] <= x.max) & 
  (pred_locs[,'y'] >= y.min & pred_locs[,'y'] <= y.max))
rf.pred = numeric(length(pred_cells))
rf.pred[idx22] = 22
rf.pred[-idx22] = as.numeric(as.character(predict(model,pred_locs[-idx22,])))
r.rf = r
r.rf[pred_cells] = rf.pred
```
